...
export class CustomerBookingController {
  static createBooking: RequestHandler = async (req, res) => {
    ...
    try {
      ...
      const customerId =
        request.user?.id || request.user?._id || request.user?.userId;
      ...
      const result = await CustomerBookingService.createBooking(customerId, {
        serviceId,
        serviceName,
        serviceImage,
        serviceCategory,
  ...
  static getMyBookings: RequestHandler = async (req, res) => {
    try {
      ...
      const bookings =
        await CustomerBookingService.getCustomerBookings(customerId);
  ...
  static getBookingById: RequestHandler = async (req, res) => {
    try {
      ...
      const booking = await CustomerBookingService.getCustomerBooking(
        customerId,
        bookingId,
      );
